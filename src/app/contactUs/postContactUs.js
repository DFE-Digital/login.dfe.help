...
const validate = (fieldsObj, services) => {
  ...
  if (
    type === "other" &&
    (!typeOtherMessage || !isValidStringValue(typeOtherMessage))
  ) {
  ...
  if (typeof typeOtherMessage === "string" && typeOtherMessage.length > 200) {
    validationMessages.typeOtherMessage =
      "Issue summary must be 200 characters or less";
  }

  if (
    !service ||
    (service &&
      ![...services.map((x) => x.name), "Other", "None"].includes(service))
  ) {
  ...
  if (!message || !isValidStringValue(message)) {
    ...
    validationMessages.message =
      "Issue details cannot be longer than 1000 characters";
  }

  if (
    !validationMessages.name &&
    !validationMessages.orgName &&
    name.toUpperCase() === orgName.toUpperCase()
  ) {
    validationMessages.name = "Full name and organisation name must not match";
    validationMessages.orgName =
      "Full name and organisation name must not match";
  }
  ...
  if (
    ...
    validationMessages.typeOtherMessage =
      "Issue summary and further details must not match";
    validationMessages.message =
      "Issue summary and further details must not match";
  }
...
const post = async (req, res) => {
  ...
  if (
    [req.body.dsiFax, req.body.dsiWebsite].some(
      (x) => typeof x !== "string" || x !== "",
    )
  ) {
    logger.audit({
  ...
  if (req.query.redirect_uri) {
    return res.redirect(
      `/contact-us/completed?redirect_uri=${req.query.redirect_uri}`,
    );
  }
